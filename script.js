  ...
  $('.showKey').on('click', function() {
    ...
    if ($('.key').html() == '') {
      ...
      for (let x = 0; x <= 11; x++) {
        keyHTML += `
        <div class='keyRow'>
          <img class='keyIcon' src='public/` + keyArray[x][0] + `.png' alt='` + keyArray[x][1] + `'>
      ...
      keyHTML += `
      <img class='slideUp' src='public/slideUp.png' alt='slide up arrow image button'>
      `; // Add slide up button
  ...
  for (let x = min; x < max; x++){
    ...
    if (!noResults) {
      let marker = {
        ...
        + ')</h1>',
        iconImage:'public/' + brewResults[x].brewery_type + '.png',
        map:map,
        label:x
      };
      addMarker(marker);
